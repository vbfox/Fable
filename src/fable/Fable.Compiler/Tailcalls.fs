// From https://github.com/tpetricek/fable-tailcalls

module Fable.Tailcalls
...
and visitExpr (f:Expr -> Choice<unit, Expr, Expr>) e =
  ...
  | Value(Lambda(args,e,isArrow)) -> Value(Lambda(args, visitExpr f e, isArrow))
  | ObjExpr(d, i, b, r) ->
    let decls = d |> List.map (fun (m, args, e) -> m, args, visitExpr f e)
    ObjExpr(decls, i, b, r)
  | IfThenElse(e1, e2, e3, r) -> IfThenElse(visitExpr f e1, visitExpr f e2, visitExpr f e3, r)
  | Switch(e, cases, def, t, r) ->
    let cases = cases |> List.map (fun (labels, body) -> List.map (visitExpr f) labels, visitExpr f body)
    Switch(visitExpr f e, cases, Option.map (visitExpr f) def, t, r)
...
type Context =
  { File : File
    TopLevel : IDictionary<string, Ident list * Expr * SourceLocation option> }
      ...
      let calls =
        decl |> foldDeclaration [] (fun calls e ->
          ...
          | Apply(Value(IdentValue(id)), _, _, _, _) when ctx.TopLevel.ContainsKey id.Name ->
              id.Name :: calls
          | Apply(Value(TypeRef(e,_)), [Value(StringConst n)], _, _, _)
...
let generateGroupFunction ctx (group:string list) =
  let funcId = Ident("_letrecFunction", Type.String)

  // Generates 'call', i.e. assigns parameters and then uses 'continue'
  // When `insertContinue' is false, we are inside a _target function and
  ...
  let makeTailCall insertContinue name args =
    ...
    let assigns =
      [0..vars.Length-1]
      // TODO: Use temp variables only if the arguments are
      // consumed after being "passed"
      |> List.map (fun i -> "_tmp" + (string i) |> makeIdent)
      |> List.zip3 vars args
    Expr.Sequential
      ( [ if hasTail group then
          ...
          if insertContinue then
            yield Expr.Continue(Some(makeIdent "_letrec"), None) ], None)

  let rec replaceTailCalls insertContinue body =
    body |> visitExpr (function
      // Declaration of match '_target' function - rather than injecting
      // 'continue' inside lambda, we inject continue _after_ the call to _target
      | VarDeclaration(id, e, mut) when id.Name.StartsWith("_target") ->
          let res = VarDeclaration(id, replaceTailCalls false e, mut)
          Choice2Of3(res)

      // Inject 'continue' call after the call to _target (which sets variables)
      | Apply(Value(IdentValue(target)), [], k, t, r) as targetCall when target.Name.StartsWith("_target") ->
          let continueExpr = Expr.Continue(Some(makeIdent "_letrec"), None)
      ...
      | Apply(Apply(Value(TypeRef(e, _)), [Value(StringConst n)], _, _, _), args, _, _, _)
        when ctx.TopLevel.ContainsKey n && e.File = ctx.File.Root.File && e.FullName = ctx.File.Root.FullName ->
  ...
  // on the value of the "_letrecFunction" parameter
  let body =
    let loopBody =
      match group with
      | [n] ->
  ...
  let loc =
    group
    |> List.choose (fun n -> let _, _, l = ctx.TopLevel.[n] in l)
  ...
  let args =
    ...
    |> List.map (function n, [g] -> g | n, _ -> makeIdent n)
  let appendMulti e l = if not(hasTail group) then l else e::l
  name, args,
    MemberDeclaration
      ( Member(name, Method, StaticLoc, appendMulti String [for a in args -> a.Type], Type.Any),
        None, appendMulti funcId args, body, Some loc)

// --------------------------------------------------------------------------------------
// Generating loops from tail-recursive calls
// --------------------------------------------------------------------------------------

let generateCall ctx fname fgroupname (fargs: Ident seq) (actualArgs: Ident seq) typ =
  ...
  Apply(makeIdentExpr fgroupname, funcId::args, ApplyMeth, typ, None)


let transform (file: File) =
  ...
  let decls =
    file.Declarations |> List.collect (function
      ...
      | d -> [d])
  File(file.SourceFile, file.TargetFile, file.Root, decls, file.IsEntry, file.UsedVarNames)
